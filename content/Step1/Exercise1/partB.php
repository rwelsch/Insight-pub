...
<p>
    <?php echo($data['description5']);?>    
</p>
<form name="step1ex1_form2" action="index.php?target=step1ex1&lang=<?php echo($lang)?>" method="POST">
    <?php 
        /*
        for($i=1;$i<11;$i++){
            echo "<br />";
            echo($data['choice']." $i: ");
            echo '<select name="step1_ex1_b_choice_'.$i.'">';
            ...
            while($line = $req->fetch()){
                ...
                $req2 = $db->query("
                    ...
                    WHERE Step1_Ex1_Users.user_id=".$_SESSION['userID']."
                      AND Step1_Ex1_Users.skill_level>1 
    ...
    <?php
        ...
        while($line = $req->fetch()){
            ...
            if($req2->rowCount() > 0){
                ...
                while($line2 = $req2->fetch()){
                    echo('<label><input type="checkbox" name="choice[]" value="'.$line2['id'].'" onclick="setChecks(this)"> '.$line2[$lang].'<br /></label>');
                }
            }
        }
        $req->closeCursor();
